...
const createCompany = async ({
  ...
  bannerUrl,
  isAutomatedJobCode,
  isAutomatedInvoiceNo,
  isRecruitForCustomers,
}: CompanyData) => {
  const params = [
    userId,
    companyName,
  ...
  const companyResponse = await executeDbQuery<Response>(async () => {
    return await query(
      `CALL usp_InsertCompanies(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, @outParam1, @outParam2, @outParam3); SELECT @outParam1 AS isSuccess, @outParam2 AS error, @outParam3 AS companyId;`,
      params,
    );
  }, "createCompany");
