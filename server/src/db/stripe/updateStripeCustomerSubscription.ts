...
const updateStripeCustomerSubscription = async ({
  userId,
  companyId,
  customerId,
  subscriptionId,
  ...
  const params = [userId, companyId, customerId, subscriptionId, signUpOn];

  const stripeCustomer = await executeDbQuery<Response>(async () => {
    return await query(
      "CALL usp_InsertCompanyPaymentDetails(?, ?, ?, ?, ?, @outParam1, @outParam2); SELECT @outParam1 AS isSuccess, @outParam2 AS error; ",
      params,
    );
