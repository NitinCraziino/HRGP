...
const verifyVerificationCodeController = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        res.status(StatusCode.OK).json({ message: "Verification code verified" });
    } catch (error) {
        next(error);
    }
};
