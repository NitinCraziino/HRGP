...
const UrlInput = ({ baseUrl, urlField, isEditing, register, errors, getValues }: UrlInputProps) => {
    return (
        <div className="mb-6">
            ...
            <div className="flex items-center border border-gray-200 rounded-md overflow-hidden">
                ...
                <div className="flex-1">
                    <input
                        ...
                        disabled={!isEditing}
                        className={`w-full border-0 focus:ring-0 outline-none py-2 px-2 ${!isEditing ? "bg-gray-50 cursor-not-allowed text-gray-500" : ""
                            }`}
                    />
                </div>
                <Button
            ...
            {errors[urlField]?.message && <p className="text-sm text-red-500 mt-1">{errors[urlField]?.message as string}</p>}
        </div>
    );
};
