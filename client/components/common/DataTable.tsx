...
export function DataTable<T>({
  ...
  const hasActions =
    actions && (actions.edit || actions.delete || (actions.custom && actions.custom.length > 0));
  ...
  const [sorting, setSorting] = useState<SortingState>(
    defaultSorting || { column: null, direction: null },
  );
        ...
        <Table className={cn("w-full", tableClassName)}>
          <TableHeader className={cn("bg-gray-100 hover:bg-gray-200", headerClassName)}>
            <TableRow>
              ...
              {hasActions && (
                <TableHead className="p-4 font-bold text-gray-600 text-center">Actions</TableHead>
              )}
            </TableRow>
          </TableHeader>
          <TableBody>
