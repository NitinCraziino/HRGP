...
const PrivatePageHeader = ({
  ...
  const breadcrumbItems = useMemo(() => {
    ...
    const matchingRoute = APPLICATION_LINKS.find(
      (route) => route.link === currentPath || currentPath.startsWith(route.link + "/"),
    );
  ...
  const pageTitle =
    title || (breadcrumbItems.length > 0 ? breadcrumbItems[breadcrumbItems.length - 1].label : "");
            ...
            <div key={item.path} className="flex items-center">
              ...
              <Link
                ...
                className={`hover:text-gray-800 ${
                  index === breadcrumbItems.length - 1 ? "text-indigo-600 font-medium" : ""
                }`}
              >
